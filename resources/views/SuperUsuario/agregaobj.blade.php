...
<body>
    ...
    <nav class="nav divportada ">
        ...
        <ul class="nav nav-pills nav-justified">
            ...
            <li class="nav-item">
                <a class="nav-link active" value="Add" id="add" href="{{ route('Agregar',['id'=>$id]) }}" disabled>Agregar objeto</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" value="Editar" id="Editar" href="{{ route('Selfmodif',['id'=>$id]) }}">Editar usuario</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="{{ route('Consultas',['id'=>$id]) }}">Consulta</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="{{ route('Encuestas',['id'=>$id]) }}">Encuestas</a>
            </li>
            <li class="nav-item">
    ...
    <div class="cuerpo">
        <div class="tabla">
            ...
            @foreach ($objetos as $objeto)
                <div class="fondoimg">
                    <button class="nbcheck" style="position: absolute; right: 85%; top:4%; background-color: rgb(218, 203, 0);"><a href="{{ route('ModifObj',['id'=>$id,'idobj'=>$objeto->id,'Tipo'=>$objeto->Tipo]) }}">
                        ...
                        salir.addEventListener('click', function() {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    window.location.href = '{{ route('ElimObj',['id'=>$id, 'idobj'=>$objeto->id]) }}';
                                }
                        });;
                        })
                    }
            ...
            <tbody>
                @foreach ($objetoselim as $objetoelim)
                    <tr>
                        <td style="width: 60%;">{{ $objetoelim->Nombre }}</td>
                        <td style="width: 10%;"><button class="nbcheck"><a href="{{ route('Recuperar',['id'=>$id,'idobj'=>$objetoelim->id]) }}">✓</a></button></td>
                    </tr>
                @endforeach
            </tbody>
            </table>
        ...
        <div class="formulario_img">
            ...
            <br><br>
            <form name="form" action="{{ route('Add', ['id'=>$id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
            <label class="textform">¿Que es?: </label>
            <input type="text" id="nombre" name="nombre" placeholder="Perro" required>
            <br><br>
            <label class="textform">Sube una imagen de eso: </label>
            <br><br>
            <input type="file" id="objeto" name="objeto" onchange="mostrarPrevia(event)" style="display: inline;" required>
            <br><br>
            {{--  
    ...
    {{--  
    <div class="fondovolver">
        <button class="confbutton" style="display: inline;"><a href="{{ route('Inicio',['id'=>$id]) }}">Volver</a></button>
    </div>
    --}}
    <script>
        document.addEventListener('DOMContentLoaded', function () {
