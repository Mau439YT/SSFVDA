...
<body>
    ...
    <nav class="nav divportada ">
        ...
        <ul class="nav nav-pills nav-justified">
            <li class="nav-item">
                <a class="nav-link active" disabled>Registro de usuarios</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" value="Salir" id="salir" href="{{ route('Consultas',['id'=>$id]) }}">Volver</a>
            </li>
        </ul>
    </nav>
    ...
    <div class="formulario">
        ...
        <br><br>
        <form id="registroform" action="{{ route('RegistroUsuario',['id'=>$id]) }}" method="POST">
             @csrf
        <label class="textform">Usuario: </label>
        <input class="form-control form-control-lg" type="text" id="usuario" name="usuario" placeholder="John Doe" value="{{ old('usuario') }}" @if (old('usuario')=='') autofocus @endif required>
        <br><br>
        <label class="textform">Email: </label>
        <input class="form-control form-control-lg" type="email"  id="email" name="email" placeholder="alguien@example.com" value="{{ old('email') }}" required>
        <br><br>
        <label class="textform">Edad: </label>
        <input class="form-control form-control-lg" type="number" id="edad" name="edad" placeholder="ej. 20" style="width: 70px; display: inline" value="{{ old('edad') }}" required>
        <br><br>
        ...
        <label class="textform" name="sx">Sexo: </label><br>
            <input class="form-control" value="Masculino" type="radio" name="radio" required @checked(old('radio')=='Masculino')><label>Masculino</label>
            <input class="form-control" value="Femenino" type="radio" name="radio" required @checked(old('radio')=='Femenino')><label>Femenino</label>
            <input class="form-control" value="Prefiere no decirlo" type="radio" name="radio" required @checked(old('radio')=='Prefiere no decirlo')><label>Prefiero no decirlo</label>
         </div>
        <br><br>
